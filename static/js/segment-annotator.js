 ...
 *
 * Author: Kota Yamaguchi 2013
 * Modified by Dmitrii Petukhov, 2015
 */

// SegmentAnnotator constructor.
SegmentAnnotator = function(segmentation, options) {
    if (typeof options === 'undefined') {
        console.log("Warning: options field is undefined.");
        options = {};
    }
...
SegmentAnnotator.prototype._getSegmentIndex = function(event) {
    var x = event.pageX - this.container.offsetLeft - this.container.parentElement.offsetLeft + this.container.scrollLeft + this.container.parentElement.scrollLeft,
        y = event.pageY - this.container.offsetTop - this.container.parentElement.offsetTop + this.container.scrollTop + this.container.parentElement.scrollTop;
...
SegmentAnnotator.prototype._updateAnnotation = function(index, render) {
    ...
    if (this.annotations.lastIndexOf(0) == -1 && document.getElementById('send-result').className == "button-disabled") {
        document.getElementById('send-result').className = "button";
        document.getElementById('send-result').onclick = function() {
            displayMessage("Sending labels...", true);
            // var dataURL = 'data:text/plain;charset=utf-8,' +
            //     encodeURIComponent(JSON.stringify(document.annotator.annotations));
            var data = {
                imageName: document.imageName,
                labels: document.annotator.annotations
            };
            console.log(JSON.stringify(data));
            $.ajax({
                type: "POST",
                url: "/api/1.0/send_labels",
                data: JSON.stringify(data),
                success: clearWorkspace,
...
SegmentAnnotator.prototype._initializeContainer = function(container) {
    ...
    var mT = $(workspace).height() / 2 - this.height / 2;
    var mL = $(workspace).width() / 2 - this.width / 2;
    if (mT < 0 || mL < 0) {
        mT = 10;
        mL = 0;
    ...
    document.getElementById('ui').style.height = this.height + mT + 'px';
